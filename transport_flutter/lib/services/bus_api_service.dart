...
class BusApiService {
  ...
  static Future<List<BusRoute>> fetchBusRoutes({String? routeName}) async {
    try {
      ...
      final uri =
          Uri.parse('${AppConfig.baseUrl}${AppConfig.fetchBusRoutesApi}')
              .replace(
        queryParameters: routeName != null && routeName.isNotEmpty
      ...
      if (response.statusCode == 200) {
        ...
        if (kDebugMode) {
          ...
          print(
              '【BusApiService】回應內容: ${response.body.substring(0, response.body.length > 500 ? 500 : response.body.length)}');
        }
        throw Exception(
      ...
      throw Exception('請求超時，後端爬蟲需要較長時間: $e');
    } catch (e) {
      if (kDebugMode) {
  ...
  static Future<BusRouteData> fetchBusRouteData(String route,
      {int direction = 0, bool forceRefresh = false}) async {
    try {
      // 建立 query parameters
      ...
      final uri = Uri.parse(
              '${AppConfig.baseUrl}${AppConfig.fetchBusRouteDataApi}${Uri.encodeComponent(route)}')
      ...
      if (response.statusCode == 200) {
        ...
        if (data['stops'] != null && data['stops'] is List) {
          ...
          for (int i = 0; i < stops.length && i < 3; i++) {
            ...
            if (kDebugMode) {
              print(
                  '  站點 $i: name=${stop['name']}, sequence=${stop['sequence']}, eta=${stop['eta']}, lat=${stop['latitude']}, lon=${stop['longitude']}');
            }
          }
        }
        ...
        for (int i = 0; i < result.stops.length && i < 3; i++) {
          ...
          if (kDebugMode) {
            print(
                '  站點 $i: ${stop.name}, lat=${stop.latitude}, lon=${stop.longitude}');
          }
        }
        ...
        throw Exception(
            'Failed to load route data for $route: ${response.statusCode} - ${response.reasonPhrase}');
      }
    } on SocketException catch (e) {
      if (kDebugMode) {
        print('【BusApiService】❌ 網路連接錯誤！');
      ...
      throw Exception('網路連接錯誤，請檢查後端服務是否運行: $e');
    } on FormatException catch (e) {
      if (kDebugMode) {
        print('【BusApiService】❌ JSON 解析錯誤！');
        print('【BusApiService】錯誤訊息: $e');
  ...
  /// [limit] 回傳數量上限，預設 20
  static Future<List<dynamic>> searchBusRoutes(String keyword,
      {int limit = 20}) async {
    try {
      final uri =
          Uri.parse('${AppConfig.baseUrl}${AppConfig.searchBusRoutesApi}')
              .replace(
        queryParameters: {
          'keyword': keyword,
